...
ADMIN_ID = 7662509798  # <--- Shuni o'zingizniki bilan almashtiring (faqat raqam)
...
# Oddiy xabarlarni tutish (Profil va Do'kon tugmalari uchun)
...
def handle_text(message):
    ...
    if message.text == "👤 Profil":
        ...
        # SIZ KIRGANINGIZDA (ADMIN) - PUL VA OLMOS HAR DOIM CHEKSIZ KO'RINADI
        if user_id == ADMIN_ID:
        ...
        profile_msg = (
            ...
            f"👤 **{message.from_user.first_name}** {'(VIP)' if user_id == ADMIN_ID else ''}\n\n"
            f"💵 Dollar: {money_txt}\n"
            f"💎 Olmos: {diam_txt}\n"
            f"🪙 Oltin: {gold_txt}\n\n"
            f"🛡 Himoya: {u['himoya']}\n"
            f"📄 Hujjat: {u['hujjat']}\n"
            f"⚖️ Osishdan himoya qilinishi: {u['ovoz_himoya']}\n"
            f"➕ Qotildan himoya: {u['qotildan_himoya']}\n"
            f"🔫 Miltiq: {u['miltiq']}\n"
            f"🪓 Doridan himoya: {u['doridan_himoya']}\n"
            f"🎭 Maska: {u['maska']}\n"
            f"🛹 Sirpanishdan himoya: {u['sirpanish_himoya']}\n"
            f"🔰 Geroydan himoya: {u['geroy_himoya']}\n\n"
            f"🎯 G'alaba: {u['wins']}\n"
            f"🎲 Barcha o'yinlar: {u['total_games']}\n\n"
            f"🎭 **Faol rollar:**\n1. {role_txt}\n2. {role_txt}"
        )
...
# ---- ⚙️ INTEGRATSIYA QILINGAN JAXONIY ADMIN BUYRUQLARI ----
...
def add_item_admin(message):
    if message.from_user.id != ADMIN_ID: return
    try:
        # Format: /plus_buyum ID buyum_nomi miqdori
        args = message.text.split()
        ...
        if item_name in SHOP_ITEMS:
            ...
            bot.send_message(target_id, f"🎁 Admin sizga {amount} ta **{item_real_name}** sovg'a qildi! Profilingizni tekshiring.", parse_mode="Markdown")
        else:
            bot.reply_to(message, "❌ Bunday buyum nomi do'konda yo'q! Ro'yxatni tekshiring.")
    except:
        bot.reply_to(message, "Format xato! `/plus_buyum ID BUYUM_NOMI MIQDOR` deb yozing.\nMisol: `/plus_buyum 123456 miltiq 3`")
...
def start_mafia(message):
    ...
    if message.from_user.id != ADMIN_ID: return
    if len(game_players) < 2: return
    
    game_started = True
    bot.send_message(message.chat.id, "🎲 Rollar lichkalarga tarqatilmoqda...")
    roles = ROLES_POOL[:len(game_players)]
    random.shuffle(roles)
    
    for i, p_id in enumerate(game_players):
        player_roles[p_id] = roles[i]
