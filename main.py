...
def start_cmd(message):
    ...
    markup.add(telebot.types.KeyboardButton("👤 Profil"), telebot.types.KeyboardButton("🛒 Do'kon"))
...
def show_shop(message):
    ...
    for k, i in SHOP_ITEMS.items():
        ...
        markup.add(telebot.types.InlineKeyboardButton(f"{i['name']} - {i['price']}{s}", callback_data=f"buy_{k}"))
    markup.add(telebot.types.InlineKeyboardButton("👑 Tungi VIP unvon — 30💎", callback_data="vip_30"))
    markup.add(telebot.types.InlineKeyboardButton(f"📈 Oltin sotib olish ({GOLD_PRICE}💵)", callback_data="gold_buy"))
    markup.add(telebot.types.InlineKeyboardButton(f"📉 Oltin sotish ({GOLD_PRICE}💵)", callback_data="gold_sell"))
...
def setup_and_start_game(message, mode):
    ...
    for i, pid in enumerate(game_players):
        ...
        if game_mode != "NOMZOD": player_names[pid] = user_data[pid]['name']
    if game_mode == "ZOMBIE":
